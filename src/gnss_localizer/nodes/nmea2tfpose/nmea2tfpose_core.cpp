...
void Nmea2TFPoseNode::publishPoseStamped()
{
  ...
  pose.pose.position.x = m_lon;
  pose.pose.position.y = m_lat;
  pose.pose.position.z = m_h;
...
void Nmea2TFPoseNode::convert(std::vector<std::string> nmea, ros::Time current_stamp)
{
  try
  {
    ...
    else if (nmea.at(0).compare(3, 3, "GGA") == 0)
    {
      ...
      std::string slat = nmea.at(2);
      std::string slon = nmea.at(4);
      double h = stod(nmea.at(9));

      geo_.set_llh_nmea_degrees(stod(slat), stod(slon), h);
      
      slat = slat.substr(0,9);
      slon = slon.substr(0,10);
      double latd, latm, lond, lonm;

      latd = stod(slat.substr(0,2));
      latm = stod(slat.substr(2,7));
      lond = stod(slon.substr(0,3));
      lonm = stod(slon.substr(3,7));

      m_lat = floor(10000 * (latd+(latm/60.0)))/10000;
    ...
    {
      position_time_ = stod(nmea.at(1));
      std::string slat = nmea.at(3);
      std::string slon = nmea.at(5);
      double h = 0.0;
